...
def run(__TEST__: bool = False) -> None:
    ...
    try:
        ...
        for table in tables:
            ...
            table_headers: list[str] = [th.get_text(strip=True) for th in t_head.find_all('th')]
            if table_headers not in kiwi_table.HEADERS.values():
                continue

            # Salva in CSV le tabelle identificate
            kiwi_table.to_csv(table, table_headers)

            # Salva in JSON le tabelle identificate
            kiwi_table.to_json(table, table_headers)

        print(f"Operazione completata. I file sono stati salvati nelle sottocartelle di {HomeKiwiOutput.CSV_DIR.name} e {HomeKiwiOutput.JSON_DIR.name}")
